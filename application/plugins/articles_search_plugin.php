		...
		if ( is_object( $this->search ) ) {
			...
			if ( is_numeric( $category_id ) AND $category_id != -1 ) {
				...
				if ( $category_id > 0 ) {
					...
					if ( isset( $cats ) AND is_array( $cats ) AND count( $cats ) > 1 ) {
			...
			if ( $this->ucm->is_logged_in() ){
				...
				if ( ! isset( $status ) ) {
					...
					if ( count( $status_condition ) > 1 ) {

						$status_condition = join( '|', $status_condition );

						$status_condition = '`t1`.`status` REGEXP "' . $status_condition . '"';
