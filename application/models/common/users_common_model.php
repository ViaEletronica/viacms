...
class Users_common_model extends CI_Model{
	...
	public function get_user_params(){
		...
		foreach ( $components as $key => $component ) {
			...
			if ( file_exists( APPPATH . 'controllers/admin/com_' . $component[ 'unique_name' ] . '/user_preferences.xml') ) {
				...
				$params = array_merge_recursive( get_params_spec_from_xml( APPPATH . 'controllers/admin/com_' . $component[ 'unique_name' ] . '/user_preferences.xml' ), $params );
	...
	public function get_users_groups_low_groups( $user = NULL ){
		...
		$user = isset( $user ) ? $user : ( $this->session->userdata( environment() . '_login' ) ? $this->user_data : NULL );
	...
	public function check_privileges( $privilege = 'admin_access' ){
		...
		if ( ! $this->session->userdata( $this->mcm->environment . '_login' ) ){
			...
			if ( check_var( $this->user_data[ 'privileges' ] ) AND gettype( $this->user_data[ 'privileges' ] ) === 'array' AND in_array( $privilege, $this->user_data[ 'privileges' ] ) ){
	...
	public function get_users_groups_privileges(){
		...
		foreach ($components as $key => $component) {
			if (file_exists(APPPATH.'controllers/admin/com_'.$component->unique_name.'/privileges.xml')) {
				$params = array_merge_recursive(get_params_spec_from_xml(APPPATH.'controllers/admin/com_'.$component->unique_name.'/privileges.xml'),$params);
			}
		}
	...
	public function do_login( $f_params = NULL ){
		...
		if ( $login_mode === 'login' AND $password AND ( $username OR $email ) ){
			...
			if ( $username ){
				...
				$user = $this->users_common_model->get_user( array( 't1.username' => $username ) )->row_array();
				
			}
			else if ( $email ){
				
				$user = $this->users_common_model->get_user( array( 't1.email' => $email ) )->row_array();
	...
	private function _make_login( $f_params ){
		...
		$user = $this->users_common_model->get_user( array( 't1.id' => $user[ 'id' ] ) )->row_array();
		...
		$this->session->set_userdata( $this->mcm->environment . '_login', TRUE );
		
		$this->session->set_userdata( $this->mcm->environment . '_login_mode', $session_mode );
		
		$this->session->set_userdata( $this->mcm->environment . '_user_data', $session_user_data );
		
		if ( $session_mode === 'persistent' ){
			
			$this->_save_access_hash( $user );
			
			if ( check_var( $user[ $this->mcm->environment . '_user_session' ] ) ){
				
				foreach ( $user[ $this->mcm->environment . '_user_session' ] as $key => $value ) {
		...
		if ( $redirect ){
			...
			// se o usuário tentou acessar uma página que não seja a de login,
			// atribui a url desta página para a variável de session uri_after_login, para o redirecionamento após o login
			if ( $this->session->userdata( $this->mcm->environment . '_uri_after_login' ) ){
				
				redirect( get_url( $this->session->userdata( $this->mcm->environment . '_uri_after_login' ) ) );
				
			}
			else redirect_last_url();
	...
	// procura pelo hash atual no banco de users, se encontrado, quer dizer que alguém se logou com a opção "Mantenha-me conectado"
	// se encontrado, retorna o usuário, caso contrário, FALSE
	...
	public function remove_access_hash( $user_hash = NULL ){
		...
		$user = $this->session->userdata( $this->mcm->environment . '_user_data' );
		$user = $this->users_common_model->get_user( array( 't1.id' => $user[ 'id' ] ) )->row_array();
		...
		if ( $user ){
			...
			if ( $this->db->update( 'tb_users', $data, array( 'id' => $user[ 'id' ] ) ) ){
				...
				$this->session->unset_userdata( $this->mcm->environment . '_access_hash' );
				return TRUE;
	...
	private function _save_access_hash( $user ){
		...
		if ( $user ){
			...
			if ( $this->db->update( 'tb_users', $data, array( 'id' => $user[ 'id' ] ) ) ){
				...
				$this->session->set_userdata( $this->mcm->environment . '_access_hash', array( 'user_hash' => $access_hash[ 'user' ], 'client_hash' => $access_hash[ 'client' ], ) );
				return TRUE;
	...
	private function _make_access_hash( $user = FALSE ){
		...
		$client_ip = ( isset( $_SERVER['HTTP_X_FORWARDED_FOR'] ) ? $_SERVER['HTTP_X_FORWARDED_FOR'] : '' ) . $_SERVER['REMOTE_ADDR'] . $_SERVER['HTTP_USER_AGENT'];
	...
	private function _make_random_username( $prefix = 'user' ){
		...
		$user = $this->users_common_model->get_user( array( 't1.username' => $prefix ) )->row_array();
	...
	function save_session_to_user()
	{
		...
		$userdata = $this->session->userdata( $this->mcm->environment . '_user_data' );
		$id = $userdata[ 'id' ];
		...
		if ( $userdata AND $id ){
			...
			$data[ 'params' ] = get_params( $user[ 'params' ] );
			$data[ 'params' ][ $this->mcm->environment . '_user_session' ] = $this->session->all_userdata();
			$data[ 'params' ] = json_encode( $data[ 'params' ] );
	...
	function remove_session_from_user()
	{
