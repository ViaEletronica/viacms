...
class Menus_mdl extends CI_Model{
	...
	public function down_menu_item_ordering( $menu_item_id = NULL ){
		...
		$menu_item = $this->get_menu_item( $menu_item_id );
		...
		if ( $alias ){
			...
			if ( $function === 'menu_items' ) {
				...
				switch ( $alias ) {
					...
					case 'select_menu_item_type': $_final_array[ 'a' ] = 'smit'; break;
	...
	public function _get_mi_url( $f_params = NULL ){
		...
		if ( ! $menu_item AND $menu_item_id ) {
			...
			if ( ! isset( $this->menu_items[ $menu_item_id ] ) ){
				...
				$menu_item = $this->menu_items[ $menu_item_id ] = $this->get_menu_item( $menu_item_id )->row_array();
	...
	public function get_menu_items_respecting_privileges( $f_params = NULL ){
		...
		if ( $this->session->userdata( environment() . '_login' ) ){
			...
			if ( $this->users_common_model->check_privileges('menus_items_can_view_all') ){
			...
			else if ( $this->users_common_model->check_privileges('menus_items_can_view_only_accessible') ){

				$gmip['where_condition'] .= ' OR ( ';

				$gmip['where_condition'] .= ' ( ';
				$gmip['where_condition'] .= ' (`access_type` = \'users\' AND `access_ids` LIKE \'%>'.$this->users_common_model->user_data['id'].'<%\')';
				$gmip['where_condition'] .= ' OR (`access_type` = \'users_groups\' AND `access_ids` LIKE \'%>'.$this->users_common_model->user_data['group_id'].'<%\')';
				$gmip['where_condition'] .= ' ) ';
				...
				$gmip['or_where_condition']['fake_index_1'] .= '( ';
				$gmip['or_where_condition']['fake_index_1'] .= '( `access_type` = \'users\' AND `access_ids` LIKE \'%>'.$this->users_common_model->user_data['id'].'<%\')';
				$gmip['or_where_condition']['fake_index_1'] .= ' OR (`access_type` = \'users_groups\' AND `access_ids` LIKE \'%>'.$this->users_common_model->user_data['group_id'].'<%\')';
				...
				$users = $this->users_common_model->get_users( $gup )->result_array();
	...
	public function view_menus_items_get_accessible_users_groups( $user_id = NULL ) {
		...
		if ($this->users_common_model->check_privileges('menus_items_can_view_only_same_and_low_group_level')){

			$accessible_groups = $this->users_common_model->get_users_groups_same_and_low_group_level();

		}
		// grupos de usuários de mesmo nível
		else if ($this->users_common_model->check_privileges('menus_items_can_view_only_same_group_level')){

			$accessible_groups = $this->users_common_model->get_users_groups_same_group_level();

		}
		else if ($this->users_common_model->check_privileges('menus_items_can_view_only_same_group_and_below')){

			$accessible_groups = $this->users_common_model->get_users_groups_same_group_and_below();

		}
		else if ($this->users_common_model->check_privileges('menus_items_can_view_only_same_group')){

			$accessible_groups = $this->users_common_model->get_users_groups_same_group();

		}
		else if ($this->users_common_model->check_privileges('menus_items_can_view_only_low_groups')){
