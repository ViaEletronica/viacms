...
class Plugins_mdl extends CI_Model{
	 ...
	 * @return mixed
	 */
	public function load( $var1 = NULL, $var2 = NULL, $var3 = NULL ){
		
		return $this->_load( $var1, $var2, $var3 );
	 ...
	 */
	private function _load( $var1 = NULL, $var2 = NULL, $var3 = NULL ){
		...
		if (
			...
			( ! isset( $name ) AND isset( $type ) AND isset( self::$_plugins[ 'by_type' ][ $type ] ) ) OR
			...
			foreach ( $plugins_source as $key => $plugin ) {
				...
				if ( $return_type === 'bool' ) {
					...
					$success = $this->_load( $plugin[ 'name' ], NULL, $return_type );
					...
					$string_return .= ( string ) $this->get_output( $plugin[ 'name' ], NULL, $return_type );
		...
		// heart's function
			...
			if ( $return_type === 'bool' ) {
				...
				$return = ( string ) $this->get_output( $name, NULL, $return_type );
	 ...
	 * @return mixed
	 */
	public function get_output( $name = NULL, $type = NULL, $return_type = 'string' ){
			...
			foreach ( self::$_plugins[ 'by_type' ][ $type ] as $key => $plugin ) {
				...
				if ( isset( self::$_plugins[ 'by_name' ][ $plugin[ 'name' ] ] ) ) {
					...
					if ( ! $is_loaded ) {
						...
						if ( ! ( $is_loaded = $this->_load( $plugin[ 'name' ], NULL, 'bool' ) ) ) break;
	 ...
	 * @return bool
	 */
	private function _run_plugin( $name = NULL, $params = NULL ){
