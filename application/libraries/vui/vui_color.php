...
class Vui_color{
	...
	function rgba_s( $opacity_index = NULL ){
		...
		$opacity_index ? $opacity_index : $this->rgba_variants;
		
		return 'rgba( ' . ( $this->rgb . ', ' . ( ( 255 / $this->rgba_variants ) * $opacity_index / 255 ) ) . ' )';
	...
	function darken( $amount = 0, $return_obj = FALSE ){
		...
		if ( $return_obj ){
			...
			if ( isset( $this->{ $new_obj_name } ) ){
				...
				$this->{ $new_obj_name } = new Vui_color( $darkerHSL );
				return $this->{ $new_obj_name };
	...
	function lighten( $amount = 0, $return_obj = FALSE ){
		...
		if ( $return_obj ){
			...
			if ( isset( $this->{ $new_obj_name } ) ){
				...
				$this->{ $new_obj_name } = new Vui_color( $lighterHSL );
				return $this->{ $new_obj_name };
	...
	private function _make_colors(){
		...
		$this->hexa_s = $this->hex_s . $this->ha;
		
		$this->rgb = $this->r . ',' . $this->g . ','  . $this->b;
		$this->rgb_s = 'rgb( ' . $this->r . ', ' . $this->g . ', '  . $this->b . ' )';
		
		$this->rgba = $this->rgb . ', '  . ( $this->a / 255 );
		$this->rgba_s = 'rgba( ' . $this->r . ', ' . $this->g . ', '  . $this->b . ', '  . ( $this->a / 255 ) . ' )';
