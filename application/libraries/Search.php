...
class Search{
	...
	private $_allowed_config = array(
		...
		'plugins_params' => NULL,
		'terms' => NULL,
		'allow_empty_terms' => TRUE, // if no terms given, decide if we mest show all results or not
		'separate_terms' => NULL,
		'ipp' => NULL,
		'cp' => NULL,
		'order_by' => NULL,
	...
	public function config( $config = NULL, $value = NULL ){
		...
		// getter
		else if ( $config AND is_string( $config ) AND ! $value ){
				...
				foreach ( $conditions as $key => $condition ) {
					...
					if ( is_numeric( $key ) ){
						...
						if ( is_array( $condition ) ){
							...
							$this->_resolve_where_conditions( $condition );
				...
				foreach ( $conditions as $key => $condition ) {
					...
					if ( is_numeric( $key ) ){
						...
						if ( is_array( $condition ) ){
							...
							$this->_resolve_or_where_conditions( $condition );
					...
					else if ( check_var( $key ) AND check_var( $condition ) ){
	...
	public function db_search( $f_params = NULL ){
		 ...
		 */
		$limit =								isset( $f_params[ 'ipp' ] ) ? ( int ) $f_params[ 'ipp' ] : ( int ) $this->config( 'ipp' );
		$offset =								isset( $f_params[ 'cp' ] ) ? ( ( int ) ( $f_params[ 'cp' ] > 0 ? $f_params[ 'cp' ] : 1 ) - 1 ) * $limit : ( ( int ) ( $this->config( 'cp' ) > 0 ? $this->config( 'cp' ) : 1 ) - 1 ) * $limit;
		$order_by_temp =						$this->config( 'order_by' );
		$order_by =								isset( $f_params[ 'order_by' ] ) ? $f_params[ 'order_by' ] : ( isset( $order_by_temp[ $plugin_name ] ) ? $order_by_temp[ $plugin_name ] : NULL ); // array / string
		...
		if ( $table_name AND $search_columns ){
			...
			if ( $order_by ) {
				...
				if ( is_string( $order_by ) ){
					...
					$this->CI->db->order_by( $order_by, '', $order_by_escape );
					...
					foreach ( $order_by as $key => $value ) {
						...
						if ( is_string( $value ) ){
							...
							$this->CI->db->order_by( $value, '', $order_by_escape );
							
						}
						else if ( is_array( $value ) ) {
							
							call_user_func_array( array( $this->CI->db, 'order_by' ), $value );
			...
			if ( $where_conditions ){
				...
				$this->_resolve_where_conditions( $where_conditions );
				
			}
			if ( $or_where_conditions ){
				
				$this->_resolve_or_where_conditions( $or_where_conditions );
	...
	public function append_results( $plugin_name = NULL, $search_results = NULL ){
		...
		if ( is_array( $search_results ) ){
			...
			if ( isset( $search_results[ 'results' ] ) ){
				...
				$this->_results[ $plugin_name ] = ( isset( $this->_results[ $plugin_name ] ) ? $this->_results[ $plugin_name ] : array() ) + $search_results[ 'results' ];
				
			}
			if ( isset( $search_results[ 'full_results' ] ) ){
				
				$this->_full_results[ $plugin_name ] = ( isset( $this->_full_results[ $plugin_name ] ) ? $this->_full_results[ $plugin_name ] : array() ) + $search_results[ 'full_results' ];
	...
	public function get_results( $plugin_name = NULL ){
			...
			foreach ( $terms as $term ) {
				...
				foreach ( $array as $i_key => & $item ) {
					...
					if ( is_array( $item ) ) {
						...
						$item = $this->array_highlight( $item, $allowed_keys );
						...
						if ( $allowed_keys ) {
							...
							$item = ( key_exists( $i_key, $allowed_keys ) AND is_string( $item ) ) ? str_highlight( $item, $term ) : $item;
							
						}
						else {
							
							$item = is_string( $item ) ? str_highlight( $item, $term ) : $item;
	...
	public function get_full_results( $plugin_name = NULL ){
		
		$return = array();
		
		if ( $this->_check_config() ) {
			
			if ( $this->_full_results === NULL ){
	...
	private function _get_full_results( $plugin_name = NULL ){
		...
		if ( isset( $this->_full_results[ $plugin_name ] ) ) {
