...
class Main extends CI_controller {
	...
	public function __construct(){
		...
		*/
		...
		if ( ! $this->session->userdata( get_constant_name( $this->mcm->environment . '_ALIAS' ) . '_last_url' ) ){
		...
		if ( $this->session->userdata( $this->mcm->environment . '_login' ) AND $this->session->userdata( $this->mcm->environment . '_user_data' ) ){
			...
			$this->users_common_model->user_data[ 'params' ] = $this->main_model->parse_params( $this->users_common_model->user_data[ 'params' ] );
			...
			$this->mcm->filtered_system_params = filter_params( $this->mcm->filtered_system_params, $this->users_common_model->user_data[ 'params' ] );
	...
	protected function _page( $f_params = NULL ){
		...
		if ( ( $component_view_folder AND $function AND $action AND $view ) OR $blank_content ){
			...
			$data[ 'user_data' ] = $this->session->userdata( $this->mcm->environment .'_user_data' );
