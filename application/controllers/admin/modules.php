...
class Modules extends Main {
	...
	public function __construct(){
		...
		if ( ! $this->users_common_model->check_privileges( 'modules_management_modules_management' ) ){
	...
	public function mm(){
		...
		if ( $action == 'ml' ){
			...
			if ( ! ( ( $order_by_direction = $this->users_common_model->get_user_preference( 'modules_list_order_by_direction' ) ) != FALSE ) ){
				
				$order_by_direction = 'ASC';
				
			}
			
			if ( ( $order_by = $this->users_common_model->get_user_preference( 'modules_list_order_by' ) ) != FALSE ){
		 ...
		 */
			...
			$this->users_common_model->set_user_preferences( array( 'modules_list_order_by' => $ob ) );
			...
			if ( ( $order_by_direction = $this->users_common_model->get_user_preference( 'modules_list_order_by_direction' ) ) != FALSE ){
				...
				$this->users_common_model->set_user_preferences( array( 'modules_list_order_by_direction' => $order_by_direction ) );
				
			}
			else {
				
				$this->users_common_model->set_user_preferences( array( 'modules_list_order_by_direction' => 'ASC' ) );
		 ...
		 */
			...
			// se o tipo do módulo estiver definido, quer dizer que o tipo de módulo foi escolhido, logo podemos prosseguir para o formulário
				...
				if ( $action == 'em' ){
					...
					// cruzando os valores padrões das especificações com os atuais
					$data[ 'module_type_final_params_values' ] = array_merge( $data[ 'module_type_params_spec' ][ 'params_spec_values' ], $data[ 'current_params_values' ] );
			...
			if ( $var1 AND ( $menu_item = $this->menus_model->get_menu_item( $var1 )->row() ) ){
				...
				$data = array( 
					...
					'users' => $this->users_common_model->get_users_checking_privileges()->result_array(),
