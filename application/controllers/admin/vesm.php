...
class Vesm extends Main {
	...
	public function __construct(){
		...
		else if ( ! $this->users_common_model->check_privileges('vesm_management_vesm_management') ){
	...
	public function tenders_management($action = NULL, $var1 = NULL, $var2 = NULL, $var3 = NULL, $var4 = NULL, $var5 = NULL, $var6 = NULL){
		...
		if ( ! $this->session->userdata( $this->mcm->environment . '_login' ) ){
			...
			if ( ! $this->users_common_model->check_privileges('companies_management_companies_management') ){
			...
			if ( $action == 'tenders_list' OR $action == 'search' ){
				...
				if ( ! ( ( $order_by_direction = $this->users_common_model->get_user_preference( 'tender_order_by_direction' ) ) != FALSE ) ){
					
					$order_by_direction = 'ASC';
					
				}
				
				if ( ( $order_by = $this->users_common_model->get_user_preference( 'tender_order_by' ) ) != FALSE ){
			...
			/***************** Change order by ****************/
				...
				$this->users_common_model->set_user_preferences( array( 'tender_order_by' => $var1 ) );
				...
				if ( ( $order_by_direction = $this->users_common_model->get_user_preference( 'tender_order_by_direction' ) ) != FALSE ){
					...
					$this->users_common_model->set_user_preferences( array( 'tender_order_by_direction' => $order_by_direction ) );
					
				}
				else {
					
					$this->users_common_model->set_user_preferences( array( 'tender_order_by_direction' => 'ASC' ) );
			...
			/**************** Add / Edit tender ***************/
				...
				if ( $action == 'add_tender' ){
					...
					$insert_data['date_creation'] = date( 'Y-m-d H:i:s', gmt_to_local( now(), $this->mcm->filtered_system_params[ 'time_zone' ] ) );
					$insert_data['date_issue'] = date( 'Y-m-d H:i:s', gmt_to_local( now(), $this->mcm->filtered_system_params[ 'time_zone' ] ) );
					...
					if ( $this->session->userdata('reference_data_id') ){
						
						$reference_data_id = $this->session->userdata('reference_data_id');
						//echo $reference_data_id;
						$this->session->unset_userdata('reference_data_id');
					...
					if ( ! $post_data ){
						...
						if ( in_array($submit_action, array('add_product'))){
							...
							$this->session->set_userdata('reference_data_id',$return_id);
	...
	public function products_select( $action = NULL, $var1 = NULL, $var2 = NULL, $var3 = NULL, $var4 = NULL, $var5 = NULL, $var6 = NULL ){
		...
		if ( $this->session->userdata('reference_data_id') ){
			...
			$this->session->set_userdata('select_on', TRUE);
