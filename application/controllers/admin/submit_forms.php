		 ...
		 */
			...
			if ( ! $post_data ){
				...
				if ( $action == 'esf' ) {
					...
					$submit_form[ 'params' ] = json_decode( $submit_form[ 'params' ], TRUE );
		 ...
		 */
			...
			if ( $submit_form_id ){
				...
				if ( ! check_var( $this->mcm->filtered_system_params[ 'admin_submit_form_users_submits_columns' ][ 'submit_form_' . $submit_form_id ] ) ){
					...
					$user_preference[ 'admin_submit_form_users_submits_columns' ] = array(
						
						'submit_form_' . $submit_form_id => $columns,
						
					);
					
					$user_preference[ 'admin_submit_form_users_submits_columns' ] = json_encode( $user_preference[ 'admin_submit_form_users_submits_columns' ] );
		...
		if ( $action == 'usl' ){
			...
			if ( $user_submit_id ){
				...
				if ( is_array( $user_submit_id ) ){
					...
					$gus_p[ 'where_condition' ] = 't1.id = ' . $user_submit_id;
					$gus_p[ 'limit' ] = 1;
					
					$user_submit = $this->submit_forms_common_model->get_users_submits( $gus_p )->row_array();
