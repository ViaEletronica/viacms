...
class Menus extends Main {
	...
	public function mim(){
		...
		if (
			...
			is_numeric( $this->users_common_model->get_user_preference( $this->mcm->environment . '_menu_items_items_per_page' ) ) AND
			$this->users_common_model->get_user_preference( $this->mcm->environment . '_menu_items_items_per_page' ) > -1 AND
			! isset( $post[ 'ipp' ] )
			
		){
			
			$ipp = $this->users_common_model->get_user_preference( $this->mcm->environment . '_menu_items_items_per_page' );
		...
		if ( $action ){
			...
			if ( $action == 'l' OR $action === 's' ){
				...
				if ( ! ( ( $order_by_direction = $this->users_common_model->get_user_preference( 'menu_items_order_by_direction' ) ) != FALSE ) ){
				...
				if ( ( $order_by = $this->users_common_model->get_user_preference( 'menu_items_order_by' ) ) != FALSE ){
				...
				// menu type
				$filter_by_menu_type = $this->users_common_model->get_user_preference( 'menus_filter_by_menu_type' );
				...
				if ( isset( $post[ 'menus_filter_by_menu_type' ] ) AND isset( $post[ 'submit_filter_by_menu_type' ] ) ){
					...
					$this->users_common_model->set_user_preferences( array( 'menus_filter_by_menu_type' => $filter_by_menu_type ) );
				...
				if ( isset( $post[ 'ipp' ] ) AND isset( $post[ 'submit_change_ipp' ] ) ){
					...
					// setting the user preference
					$this->users_common_model->set_user_preferences( array( $this->mcm->environment . '_menu_items_items_per_page' => $post[ 'ipp' ] ) );
			 ...
			 */
				...
				$this->users_common_model->set_user_preferences( array( 'menu_items_order_by' => $order_by ) );
				...
				if ( ( $order_by_direction = $this->users_common_model->get_user_preference( 'menu_items_order_by_direction' ) ) != FALSE ){
					...
					$this->users_common_model->set_user_preferences( array( 'menu_items_order_by_direction' => $order_by_direction ) );
					
				}
				else {
					
					$this->users_common_model->set_user_preferences( array( 'menu_items_order_by_direction' => 'ASC' ) );
			...
			else if ( $action == 'a' OR $action == 'smit' OR $action == 'e' ){
