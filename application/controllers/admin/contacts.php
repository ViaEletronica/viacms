...
class Contacts extends Main {
	...
	public function contacts_management($action = NULL, $var1 = NULL, $var2 = NULL, $var3 = NULL, $var4 = NULL, $var5 = NULL, $var6 = NULL){
		...
		if ( $action != 'edit_contact' AND ! $this->users_common_model->check_privileges('contacts_management_contacts_management') ){
		...
		/***************** Google contacts ****************/
			...
			$service_sub = $this->users_common_model->user_data[ 'email' ];
				...
				foreach ( $response as $key => $resp ) {
					...
					if ( $key == 'error' ){
						...
						if ( $resp[ 'code' ] == '401' AND $resp[ 'message' ] == 'Login Required'  ){
							...
							$this->session->unset_userdata( 'token' );
							$this->session->set_userdata( 'google_error_login_required', TRUE );
							$auth = $client->createAuthUrl();
							redirect( $auth );
							...
							$this->session->unset_userdata( 'token' );
							$this->session->set_userdata( 'google_error_invalid_credentials', TRUE );
							$auth = $client->createAuthUrl();
							redirect( $auth );
