	...
	if ( $CI->mcm->filtered_system_params[ 'friendly_urls' ] AND ( isset( $reverse_urls ) AND is_array( $reverse_urls ) ) ){
		...
		if ( array_key_exists( $original_url, $reverse_urls ) ){
			...
			if ( $component_item == 'article_detail' ){
				...
				if ( empty( $url ) AND $article = $CI->articles->get_articles_respecting_privileges( $gap )->row_array() ){
					
					$category = $CI->articles_model->get_category( $article[ 'category_id' ] )->row_array();
					...
					$tb_urls_data = array(
						...
						'target' => $CI->articles->get_link_article_detail( current_menu_id(), $article[ 'id' ], $article[ 'category_id' ] )
				...
				if ( $category_id == '-1' OR $category_id == '0' OR $category = $CI->articles_model->get_category( $category_id )->row_array() ){
					...
					if ( empty( $url ) ){
						...
						$tb_urls_data = array(
							...
							'target' => $CI->articles->get_link_articles_list( current_menu_id(), $category_id, $u, TRUE, TRUE, $p, $ipp ), // <<<<<<<<<<=======############ AAAAAQQQQQUIIIIIII
