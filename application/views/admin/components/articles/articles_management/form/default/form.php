					...
					<fieldset class="for-tab">
						...
						<div id="title" class="vui-field-wrapper-inline">
							...
							<?php
								...
								$field_attr = 'autofocus id="article-' . $field_name . '" name="' . $field_name . '" class="' . $field_name . ' ' . ( $field_error ? 'field-error' : '' ) . '"' . ( $field_error ? element_title( $field_error ) : '' );
						...
						<div id="category" class="vui-field-wrapper-inline">
							...
							<?= form_dropdown( $field_name, $field_options, set_value( $field_name, @$article[ $field_name ] ?@$article[ $field_name ] : ( $this->users_common_model->get_user_preference( 'articles_filter_by_category' ) ? $this->users_common_model->get_user_preference( 'articles_filter_by_category' ) : 0 ) ), $field_attr ); ?>
...
<script type="text/javascript">
	...
	$( document ).bind( 'ready', function(){
		...
		$( '#article-image' ).after( '<?= vui_el_button( array( 'attr' => 'data-rffieldid="article-image" data-rftype="image"', 'url' => '#', 'text' => lang( 'select_image' ), 'get_url' => FALSE, 'id' => 'image-picker', 'icon' => 'more', 'only_icon' => TRUE, 'class' => 'modal-file-picker', ) ); ?>' );
