...
if ( ! defined( 'BASEPATH' ) ) {
	...
	$system_path = dirname($_SERVER['SCRIPT_FILENAME']) . DIRECTORY_SEPARATOR . 'system';
	$system_path = str_replace( 'assets/js/responsivefilemanager/filemanager/config/', '', $system_path );
	$application_folder = dirname($_SERVER['SCRIPT_FILENAME']) . DIRECTORY_SEPARATOR . 'application';
	$application_folder = str_replace( 'assets/js/responsivefilemanager/filemanager/config/', '', $application_folder );
	
	$system_path = rtrim($system_path, '/') . '/';
	$system_path = str_replace( 'assets/js/responsivefilemanager/filemanager/', '', $system_path );
	...
	$fcpath = str_replace( SELF, '', $_SERVER['SCRIPT_FILENAME'] );
...
$constants_file = BASE_PATH . 'application' . DS . 'config' . DS . 'constants.php';
$constants_file = str_replace( '/uploader/', '', $constants_file );

require_once $constants_file;

$ci_config_file = BASE_PATH . 'application' . DS . 'config' . DS . 'config.php';
$ci_config_file = str_replace( '/uploader/', '', $ci_config_file );

require_once $ci_config_file;
...
$base_url = HTTP_HOST; // DON'T TOUCH (base url (only domain) of site (without final /)).
$upload_dir = RELATIVE_BASE_URL . '/'; // path from base_url to base of upload folder (with start and final /)
...
$access_keys = array( md5( $config['encryption_key'] ) );
